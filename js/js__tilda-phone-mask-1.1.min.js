function t_form_phonemask_load_one(t) { $("#record-" + t).find(".js-phonemask-input").each(function() { t_form_phonemask_load($(this)) }) }

function t_form_phonemask_load(t) {
    var a = t.attr("data-phonemask-id"),
        p = t.attr("data-phonemask-lid"),
        n = t.attr("data-phonemask-maskcountry") || "";
    "undefined" != n && n && 0 != n.length || (n = ""), t.attr("data-phonemask-init", "yes"), setTimeout(function() { try { t_form_phonemask_init(a, p, n) } catch (t) { console.log(t) } }, 1e3)
}

function t_form_phonemask_init(a, p, t) { void 0 !== t && "" != t ? t_form_phonemask_do(a, p, t) : void 0 !== window.geo_iso && "" != window.geo_iso && null != window.geo_iso ? t_form_phonemask_do(a, p, window.geo_iso) : $.ajax({ type: "GET", url: "https://geo.tildacdn.com/geo/country/", success: function(t) { window.geo_iso = t, t_form_phonemask_do(a, p, t) }, error: function() { t_form_phonemask_do(a, p, "") }, timeout: 5e3 }) }

function t_form_phonemask_do(t, a, p) {
    var n = $("#record-" + t).find('[data-input-lid="' + a + '"]');
    0 == n.length && (void 0 !== window.tildamode && "zero" == window.tildamode ? 0 == (n = $(".tn-artboard").find('[data-input-lid="' + a + '"]')).length && (n = $(".tn-artboard")) : 0 == (n = $("body").find('[data-input-lid="' + a + '"]')).length && (n = $("body"))), t_form_phonemask_replace_input(n), $(".t-input-phonemask__wrap").hasClass("t-input-phonemask__wrap-style") || t_form_phonemask_add_style();
    var o = n.find(".t-input-phonemask");
    "string" == typeof p && null != p || (console.log("Error. ISO is not string or null: " + p), p = ""), 0 < p.length && (p = p.toLowerCase()), 2 < p.length && (console.log("Error ISO: " + p), p = "");
    var i = t_form_phonemask_getCountriesList();
    if (void 0 !== i[p]) var e = i[p];
    else e = { n: "Russian Federation (Российская Федерация)", c: "+7", m: "+7(999) 999-99-99" }, p = "ru";
    o.attr("data-phonemask-code", 0 < p.length ? e.c : ""), o.attr("data-phonemask-mask", 0 < p.length ? e.m : ""), t_form_phonemask_calc_maxlength(o), 0 < p.length && (localStorage.setItem("iso_code", p), n.find(".t-input-phonemask__select-flag").attr("data-phonemask-flag", p), n.find(".t-input-phonemask__select-code").html(e.c), void 0 !== o.attr("data-phonemask-mask") && o.attr("placeholder", o.attr("data-phonemask-mask").substr(o.attr("data-phonemask-code").length).replace(/^-+/, "")), t_form_phonemask_change_min_length(o.attr("placeholder"), o.attr("data-phonemask-code"), n)), t_form_phonemask_add_current_style(n), n.find(".t-input-phonemask__options-wrap").html(t_form_phonemask_draw_selector(i)), n.find(".t-input-phonemask__select").on("click", function() { $(".t-input-phonemask__options-wrap").removeClass("t-input-phonemask__options-wrap_open"), $(this).siblings(".t-input-phonemask__options-wrap").toggleClass("t-input-phonemask__options-wrap_open") }), n.find(".t-input-phonemask__options-item").on("click", function() { t_form_phonemask_choose_country(n, a, $(this)), t_form_phonemask_calc_maxlength(o) }), o.on("input", function() {
        var t = $(this);
        t.attr("data-phonemask-current", t.val()), t_form_phonemask_add_number_mask(t_form_phonemask_copypaste_handling(t, o.attr("data-phonemask-without-code")), $(this)), t_form_phonemask_change_val(o, n)
    }), document.onmouseup = function(t) {
        var a = $(".t-input-phonemask__options-wrap");
        null === t.target.closest(".t-input-phonemask__options-wrap") && null === t.target.closest(".t-input-phonemask__select") && a.removeClass("t-input-phonemask__options-wrap_open")
    ...
    $(document).on("keyup", function(p) {
        $(".t-input-phonemask__options-wrap").each(function() {
            if ($(this).hasClass("t-input-phonemask__options-wrap_open")) {
                var t = [];
                if ($(".t-input-phonemask__options-item").removeClass("t-input-phonemask__options-item_chosen"), 48 <= p.keyCode && p.keyCode <= 57 || 96 <= p.keyCode && p.keyCode <= 105)
                    for (var a in s += p.key, i) t_form_phonemask_search_country($(this), s, t_form_phonemask_regexp_replace(s), t, i, a, i[a].c);
                else if (65 <= p.keyCode && p.keyCode <= 90 || 32 == p.keyCode)
                    for (var a in s += p.key, i) t_form_phonemask_search_country($(this), s, "^" + t_form_phonemask_regexp_replace(s), t, i, a, i[a].n);
                clearTimeout(h), h = setTimeout(function() { s = "" }, 1500)
            }
        })
    })
}

function t_form_phonemask_calc_maxlength(t) { var a = t.attr("data-phonemask-mask").substr(t.attr("data-phonemask-code").length); "-" == a.charAt(0) && (a = a.substring(1)), t.attr("maxlength", a.length), t.attr("data-phonemask-without-code", a) }

function t_form_phonemask_copypaste_handling(t, a) {
    var p = t.val(),
        n = null != p.match(/[0-9]/g) ? p.match(/[0-9]/g) : p;
    return n.length > a.match(/[0-9]/g).join("").length && (p = n.splice(t.attr("data-phonemask-code").match(/[0-9]/g).length).join("")), p
}

function t_form_phonemask_regexp_replace(t) { return t.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&") }

function t_form_phonemask_search_country(t, a, p, n, o, i, e) {
    var s = new RegExp(p, "i");
    if (-1 != e.search(s)) {
        n.push(t.find("#" + o[i].id)), n[0].addClass("t-input-phonemask__options-item_chosen");
        var h = t;
        h.animate({ scrollTop: t.find(".t-input-phonemask__options-item_chosen").offset().top - h.offset().top + h.scrollTop() }, 0)
    }
}
