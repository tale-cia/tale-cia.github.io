(function(factory) { if (typeof define === 'function' && define.amd) { define(['jquery'], factory) } else { if (typeof exports === 'object') { factory(require('jquery')) } else { factory(jQuery) } } }(function($) {
    var ua = navigator.userAgent,
        ...
        caretTimeoutId;
    $.mask = { definitions: { '9': "[0-9]", 'a': "[A-Za-z]", 'а': "[А-Яа-яЁё]", '*': "[A-Za-zА-Яа-яЁё0-9]" }, autoclear: !0, dataName: "rawMaskFn", placeholder: '_' };
    $.fn.extend({
        caret: function(begin, end) {
            var range;
            if (this.length === 0 || this.is(":hidden") || this.get(0) !== document.activeElement) { return }
            if (typeof begin == 'number') { end = (typeof end === 'number') ? end : begin; return this.each(function() { if (this.setSelectionRange) { this.setSelectionRange(begin, end) } else { if (this.createTextRange) { range = this.createTextRange();
                            range.collapse(!0);
                            range.moveEnd('character', end);
                            range.moveStart('character', begin);
                            range.select() } } }) } else {
                if (this[0].setSelectionRange) { begin = this[0].selectionStart;
                    end = this[0].selectionEnd } else { if (document.selection && document.selection.createRange) { range = document.selection.createRange();
        ...
        mask: function(mask, settings) {
            ...
            return this.trigger("unmask").each(function() {
                var input = $(this),
                    buffer = $.map(mask.split(""), function(c, i) { if (c != '?') { return defs[c] ? getPlaceholder(i) : c } }),
                ...
                function shiftL(begin, end) {
                    ...
                    for (i = begin, j = seekNext(end); i < len; i++) {
                        if (tests[i]) {
                            if (j < len && tests[i].test(buffer[j])) { buffer[i] = buffer[j];
                                buffer[j] = getPlaceholder(j) } else { break }
                            j = seekNext(j)
                        }
                    }
                    writeBuffer();
                ...
                function shiftR(pos) { var i, c, j, t; for (i = pos, c = getPlaceholder(pos); i < len; i++) { if (tests[i]) { j = seekNext(i);
                            t = buffer[i];
                            buffer[i] = c; if (j < len && tests[j].test(t)) { c = t } else { break } } } }
                ...
                function androidInputEvent(e) {
                    ...
                    if (oldVal && oldVal.length && oldVal.length > curVal.length) {
                        ...
                        var curPos = pos.end;
                        while (curPos > 0 && !tests[curPos - 1]) { curPos-- }
                        if (curPos === 0) { curPos = nextPos }
                        pos.begin = curPos;
                        setTimeout(function() { proxy();
                            tryFireCompleted() }, 0)
                    } else { pos.begin = checkVal(!0);
                        setTimeout(function() { proxy();
                            tryFireCompleted() }, 0) }
                }
                ...
                function keydownEvent(e) {
                    ...
                    if (k === 8 || k === 46 || (iPhone && k === 127)) {
                        ...
                        shiftL(begin, end - 1);
                        e.preventDefault()
                    } else { if (k === 13) { blurEvent.call(this, e) } else { if (k === 27) { input.val(focusText);
                ...
                function keypressEvent(e) {
                    ...
                    var k = e.which || e.keyCode,
                        pos = input.caret(),
                        p, c, next;
                    if (e.ctrlKey || e.altKey || e.metaKey || k < 32) { return } else {
                        if (k && k !== 13) {
                            ...
                            if (p < len) {
                                ...
                                if (tests[p].test(c)) {
                                    ...
                                    writeBuffer();
                                    next = seekNext(p);
                                    if (android) { var proxy = function() { $.proxy($.fn.caret, input, next)() };
                                        setTimeout(proxy, 0) } else { input.caret(next) }
                ...
                function checkVal(allow) {
                    ...
                    for (i = 0, pos = 0; i < len; i++) {
                        if (tests[i]) {
                            ...
                            while (pos++ < test.length) { c = test.charAt(pos - 1); if (tests[i].test(c)) { buffer[i] = c;
                                    lastMatch = i; break } }
                    ...
                    if (allow) { writeBuffer() } else { if (lastMatch + 1 < partialPosition) { if (settings.autoclear || buffer.join('') === defaultBuffer) { if (input.val()) input.val("");
                                clearBuffer(0, len) } else { writeBuffer() } } else { writeBuffer();
                ...
                input.one("unmask", function() { input.off(".mask").removeData($.mask.dataName) }).on("focus.mask", function() {
                    ...
                    caretTimeoutId = setTimeout(function() {
                        ...
                        writeBuffer();
                        if (pos == mask.replace("?", "").length) { input.caret(0, pos) } else { input.caret(pos) }
                    }, 10)
                }).on("blur.mask", blurEvent).on("keydown.mask", keydownEvent).on("keypress.mask", keypressEvent).on("input.mask paste.mask", function() {
                    if (input.prop("readonly")) { return }
                    setTimeout(function() { var pos = checkVal(!0);
                        input.caret(pos);
                        tryFireCompleted() }, 0)
                });
...
(function($) {
    ...
    if (window.tildaBrowserLang.indexOf('RU') != -1) { window.tildaBrowserLang = 'RU' } else { window.tildaBrowserLang = 'EN' }
    ...
    $(document).ready(function() {
                ...
                if (req == 1 && ((val.length == 0 && valnospace.length == 0) || (($(this).attr('type') == 'checkbox' || $(this).attr('type') == 'radio') && $(this).closest('form').find('[name="' + $(this).attr('name') + '"]:checked').length == 0))) { error.type.push('req') } else {
                    switch (rule) {
                        case 'email':
                            ...
                            if (val.length > 0 && !val.match(regExp)) { error.type.push('email') }
                            break;
                        case 'url':
                            regExp = /^((https?|ftp):\/\/)?[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9_\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}\/?$/gi;
                            if (val.length > 0) {
                                str = val.split('//');
                                if (str && str.length > 1) { str = str[1] } else { str = str[0] }
                                str = str.split('/');
                                if (str && str.length > 0 && str[0] > '') { str = str[0]; if (!str.match(regExp)) { error.type.push('url') } } else {
                        ...
                        case 'phone':
                            regExp = /^[0-9\(\)\-\+]+$/gi;
                            if (valnospace.length > 0 && !valnospace.match(regExp)) { error.type.push('phone') } else { str = valnospace.replace(/[^0-9]+/g, ''); if (str.indexOf('000') == 1 || str.indexOf('111') == 1 || (str.indexOf('222') == 1 && str.substring(0, 1) != '5') || str.indexOf('333') == 1 || str.indexOf('444') == 1 || (str.indexOf('555') == 1 && str.substring(0, 1) != '0') || (str.indexOf('666') == 1 && str.substring(0, 1) != '0') || (str.indexOf('8888') == 1 && str.substring(0, 1) != '4')) { error.type.push('phone') } }
                            break;
                        case 'number':
                            regExp = /^[0-9]+$/gi;
                            if (valnospace.length > 0 && !valnospace.match(regExp)) { error.type.push('number') }
                            break;
                        case 'date':
                            regExp = /^[0-9]{1,4}[\-\.\/][0-9]{1,2}[\-\.\/][0-9]{1,4}$/gi;
                            if (valnospace.length > 0 && !valnospace.match(regExp)) { error.type.push('date') }
                            break;
                        case 'time':
                            regExp = /^[0-9]{2}[:\.][0-9]{2}$/gi;
                            if (valnospace.length > 0 && !valnospace.match(regExp)) { error.type.push('time') }
                            break;
                        case 'name':
                            ...
                            if (val.length > 0 && !val.match(regExp)) { error.type.push('name') }
                            break;
                        case 'nameeng':
                            regExp = /^([A-Za-z\s]{1,}((\-)?[A-Za-z\.\s](\')?){0,})*$/i;
                            if (val.length > 0 && !val.match(regExp)) { error.type.push('nameeng') }
                            break;
                        case 'namerus':
                            regExp = /^([А-Яа-яЁё\s]{1,}((\-)?[А-Яа-яЁё\.\s](\')?){0,})*$/i;
                            if (val.length > 0 && !val.match(regExp)) { error.type.push('namerus') }
                            break;
                        case 'string':
                            ...
                            if (val.length > 0 && !val.match(regExp)) { error.type.push('string') }
                            break;
                        default:
                            break
                    }
        ...
        window.tildaForm.hideErrors = function($jform) { $jform.find('.js-errorbox-all').hide();
            ...
            $jform.removeClass('js-send-form-success'); var $popupError = $('#tilda-popup-for-error'); if ($popupError.length > 0) { $popupError.fadeOut() } };
        window.tildaForm.showErrorInPopup = function($jform, arErrors) {
            ...
            if (!clsInputBoxSelector) { clsInputBoxSelector = '.blockinput' }
            ...
            if ($popupError.length == 0) { $('body').append('<div id="tilda-popup-for-error" class="js-form-popup-errorbox tn-form__errorbox-popup" style="display: none;"> <div class="t-form__errorbox-text t-text t-text_xs"> error </div> <div class="tn-form__errorbox-close js-errorbox-close"> <div class="tn-form__errorbox-close-line tn-form__errorbox-close-line-left"></div> <div class="tn-form__errorbox-close-line tn-form__errorbox-close-line-right"></div> </div> </div>');
                ...
                $('#tilda-popup-for-error').on('click', '.js-errorbox-close', function(e) { e.preventDefault();
                    $('#tilda-popup-for-error').fadeOut(); return !1 }) }
            sCommonError = '';
            for (var i = 0; i < arErrors.length; i++) {
                ...
                if ($fieldgroup.find('.t-input-error').length > 0) { isFieldErrorBoxExist = 1 }
                for (j = 0; j < arErrors[i].type.length; j++) {
                    error = arErrors[i].type[j];
                    sError = '';
                    if (isShowCommonErrors == 1) { $errItem = $jform.find('.js-rule-error-' + error); if ($errItem.length > 0) { if ($errItem.text() == '' && arLang[error] > '') { if (sCommonError.indexOf(arLang[error]) == -1) { sCommonError = sCommonError + '<p class="t-form__errorbox-item">' + arLang[error] + '</p>' } } else { sError = $errItem.eq(0).text(); if (sCommonError.indexOf(arLang[error]) == -1) { sCommonError = sCommonError + '<p class="t-form__errorbox-item">' + $errItem.eq(0).text() + '</p>' } } } else { if (arLang[error] > '') { if (sCommonError.indexOf(arLang[error]) == -1) { sCommonError = sCommonError + '<p class="t-form__errorbox-item">' + arLang[error] + '</p>' } } } }
            ...
            if (sCommonError > '') { $popupError.find('.t-form__errorbox-text').html(sCommonError);
            ...
            window.errorTimerID = window.setTimeout(function() { $('#tilda-popup-for-error').fadeOut();
                $jform.find('.t-input-error').html('').fadeOut();
                window.errorTimerID = 0 }, 10000);
            $jform.off('focus change', '.js-tilda-rule');
            $jform.on('focus change', '.js-tilda-rule', function() { $('#tilda-popup-for-error').fadeOut();
                $(this).closest('form').find('.t-input-error').html('').fadeOut(); if (window.errorTimerID) { window.clearTimeout(window.errorTimerID);
        ...
        window.tildaForm.showErrors = function($jform, arErrors) {
            ...
            for (var i = 0; i < arErrors.length; i++) {
                if (!arErrors[i] || !arErrors[i].obj) { continue }
                if (i == 0 && arErrors[i].obj == 'none') { $errItem = $jform.find('.js-rule-error-all');
                    $errItem.html(arLang.emptyfill);
                    $errItem.css('display', 'block').show(); break }
                $fieldgroup = arErrors[i].obj.closest(clsInputBoxSelector).addClass('js-error-control-box')
                isFieldErrorBoxExist = 0;
                isShowCommonErrors = 1;
                if ($fieldgroup.find('.t-input-error').length > 0) { isFieldErrorBoxExist = 1 }
                for (j = 0; j < arErrors[i].type.length; j++) {
                    ...
                    if (isShowCommonErrors == 1) {
                        ...
                        } else { if (arLang[error] > '') { $errItem = $jform.find('.js-rule-error-all'); if ($errItem && $errItem.length > 0) { $errItem.html(arLang[error]);
                                    $errItem.css('display', 'block').show() } } }
                    }
                    if (isFieldErrorBoxExist) {
                        if (sError == '') { if (arLang[error + 'field'] > '') { sError = arLang[error + 'field'] } else { if (arLang[error] > '') { sError = arLang[error] } } }
        ...
        checkVerifyTildaCaptcha = function(event) {
            if (event.origin.indexOf('forms.tildacdn.com') != -1) {
                if (event.data == 'closeiframe') { $('#tildaformcaptchabox').remove();
                    $('#js-tildaspec-captcha').remove(); return }
                var capthakey = event.data;
                var $elem = $('#js-tildaspec-captcha');
                if ($elem && $elem.length > 0) { $elem.val(capthakey);
        ...
        window.tildaForm.addTildaCaptcha = function($jform, formskey) {
            ...
            var randomkey;
            try { randomkey = '' + new Date().getTime() + '=' + parseInt(Math.random() * 8) } catch (e) { randomkey = 'rnd=' + parseInt(Math.random() * 8) }
        ...
        window.tildaForm.addPaymentInfoToForm = function($jform) {
            ...
            if (currencyCode) { window.tildaForm.currencyForStat = currencyCode;
                window.tildaForm.tildapayment.currency = currencyCode } else { if (window.tcart.currency && window.tcart.currency > '') { window.tildaForm.currencyForStat = window.tcart.currency;
            ...
            if (window.tcart.system && window.tcart.system > '') { window.tildaForm.tildapayment.system = window.tcart.system } else { window.tildaForm.tildapayment.system = 'auto' }
            if (window.tcart.promocode && window.tcart.promocode.promocode > '') {
                ...
                if (window.tcart.prodamount_discountsum && parseFloat(window.tcart.prodamount_discountsum) > 0) { window.tildaForm.tildapayment.discount = window.tcart.prodamount_discountsum;
                    proddiscount = window.tcart.prodamount_discountsum } else { if (window.tcart.amount_discountsum && parseFloat(window.tcart.amount_discountsum) > 0) { proddiscount = window.tcart.amount_discountsum;
            ...
            if (window.tcart.products && window.tcart.products.length > 0) { iProductsCount = window.tcart.products.length }
            ...
            for (i = 0; i < iProductsCount; i = i + 1) {
                    ...
                    if (typeof(product[j]) != 'function') {
                        if (j == 'options') {
                            ...
                            for (var o in product[j]) {
                                if (typeof window.tildaForm.tildapayment.products[i][j][o] == 'undefined') { window.tildaForm.tildapayment.products[i][j][o] = {} }
                ...
                if (product.price) { arProduct.price = product.price;
                    arProduct.quantity = parseInt(product.amount / product.price) } else {
                    if (product.quantity && product.quantity > 1) { arProduct.price = product.amount / product.quantity;
                        arProduct.quantity = product.quantity } else {
                        arProduct.price = product.amount
                        arProduct.quantity = 1
                    }
                }
                arProduct.name = product.name;
                if (optionlabel > '') { arProduct.name = arProduct.name + '(' + optionlabel + ')' }
            ...
            if (window.tcart.delivery && window.tcart.delivery.name) {
                ...
                if (window.tcart.delivery && window.tcart.delivery.price) {
                    ...
                    if (window.tcart.prodamount > 0 && typeof window.tcart.delivery.freedl != 'undefined' && window.tcart.delivery.freedl > 0) { window.tildaForm.tildapayment.delivery.freedl = window.tcart.delivery.freedl; if ((window.tcart.prodamount - proddiscount) >= window.tcart.delivery.freedl) { pricedelivery = 0 } }
        ...
        window.tildaForm.clearTCart = function($jform) {
            if ($jform.data('formcart') == 'y') {
                ...
                if (typeof localStorage === 'object') { try { localStorage.removeItem("tcart") } catch (e) { console.log('Your web browser does not support localStorage.') } }
                try { delete window.tcart;
                    tcart__loadLocalObj() } catch (e) {}
                window.tcart_success = 'yes'
            }
        }
        window.tildaForm.payment = function($jform, arNext) {
            var html = '';
            if ($jform.data('formpaymentoff') == 'y') { tildaForm.clearTCart($jform); return }
            if ($jform.find('.js-successbox').length > 0) {
                ...
                if (arMessage.successorder) { $jform.find('.js-successbox').html(arMessage.successorder) }
                $jform.find('.js-successbox').show()
            }
            $jform.addClass('js-send-form-success');
            if (arNext.type == 'link') {
                tildaForm.clearTCart($jform);
                if (arNext.message && arNext.message > '' && $jform.find('.js-successbox').length > 0) { $jform.find('.js-successbox').html(arNext.message);
                    $jform.find('.js-successbox').show() }
                ...
                if (arNext.type == 'form') {
                    ...
                    arNext.value.action = '';
                    for (key in arNext.value) { val = arNext.value[key]; if (typeof(val) != 'function' && val > '') { html += "<input type='hidden' name='" + key + "' value='" + val + "' >" } }
                    html += '</form>';
                    $('body').append(html);
                    ...
                    if (arNext.type == 'function') {
                        ...
                        if (arNext.value.functioncode) { tildaForm.paysystemRun(arNext.value.script, arNext.value.sysname, $jform, arNext.value.functioncode, arArgs) } else { eval(arNext.value.name + '($jform, arArgs);') }
                        return !1
                    } else { tildaForm.clearTCart($jform); if (arNext.type == 'text' && arNext.message && arNext.message > '' && $jform.find('.js-successbox').length > 0) { $jform.find('.js-successbox').html(arNext.message);
                            $jform.find('.js-successbox').show() } }
                }
            }
        };
        window.tildaForm.paysystemScriptLoad = function(src, sysname) {
            ...
            if (!window.scriptSysPayment[sysname] || window.scriptSysPayment[sysname] !== !0) { var script = document.createElement("script");
        ...
        window.tildaForm.paysystemRun = function(script, sysname, $jform, functioncode, arArgs) {
            if (!window.scriptSysPayment) { window.scriptSysPayment = {} }
            if (!window.scriptSysPayment[sysname] || window.scriptSysPayment[sysname] !== !0) { window.tildaForm.paysystemScriptLoad(script, sysname);
                window.setTimeout(function() { window.tildaForm.paysystemRun(script, sysname, $jform, functioncode, arArgs) }, 200); return !1 }
            eval(functioncode)
        };
        window.tildaForm.paysystemSuccess = function($jform, arArgs) {
            ...
            if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') {
                ...
                if (!currency) { $('#allrecords').data('tilda-currency', arArgs.currency) }
        ...
        window.tildaForm.stripeLoad = function() { if (window.stripeapiiscalled !== !0) { var script = document.createElement("script");
                script.type = "text/javascript";
                script.src = "https://checkout.stripe.com/checkout.js";
                document.body.appendChild(script);
                window.stripeapiiscalled = !0 } };
        window.tildaForm.stripePay = function($jform, arArgs) {
            if (window.stripeapiiscalled !== !0) { window.tildaForm.stripeLoad();
                window.setTimeout(function() { window.tildaForm.stripePay($jform, arArgs) }, 200); return !1 }
            var companyname = arArgs.companyname;
            var companylogo = arArgs.companylogo;
            if (!companyname) { companyname = window.location.host }
            if (!window.stripehandler) {
                if (typeof window.StripeCheckout != 'object') { window.setTimeout(function() { window.tildaForm.stripePay($jform, arArgs) }, 200); return !1 }
                var arStripeInit = { key: arArgs.accountid, image: companylogo, name: companyname, locale: 'auto' };
                if (arArgs.zipCode && arArgs.zipCode == 1) { arStripeInit.zipCode = !0 }
            ...
            var multiple = 100;
            try { if (arArgs.multiple && arArgs.multiple > 0) { multiple = parseInt(arArgs.multiple) } } catch (e) {}
            window.stripehandler.open({
                name: companyname,
                image: companylogo,
                description: arArgs.description,
                amount: parseInt(parseFloat(arArgs.amount) * multiple),
                ...
                token: function(token, args) {
                    if (token && token.id) {
                        $.ajax({
                            ...
                            data: { projectid: arArgs.projectid, invoiceid: arArgs.invoiceid, token: token.id, email: token.email, currency: arArgs.currency, amount: parseInt(parseFloat(arArgs.amount) * multiple) },
                            dataType: "json",
        ...
        window.tildaForm.cloudpaymentLoad = function() { if (window.cloudpaymentsapiiscalled !== !0) { var script = document.createElement("script");
                script.type = "text/javascript";
                script.src = "https://widget.cloudpayments.ru/bundles/cloudpayments";
                document.body.appendChild(script);
                window.cloudpaymentsapiiscalled = !0 } };
        window.tildaForm.cloudpaymentPay = function($jform, arArgs) {
            if (window.cloudpaymentsapiiscalled !== !0) { window.tildaForm.cloudpaymentLoad();
                window.setTimeout(function() { window.tildaForm.cloudpaymentPay($jform, arArgs) }, 200); return !1 }
            var currency = arArgs.currency;
            var language = arArgs.language;
            var initCP = {};
            if (!language) { if (currency == 'RUB' || currency == 'BYR' || currency == 'BYN' || currency == 'RUR') { language = 'ru-RU' } else { if (currency == 'UAH') { language = 'uk' } else { language = 'en-US' } } }
            if (!window.cloudpaymentshandler) {
                if (typeof window.cp != 'object') { window.setTimeout(function() { window.tildaForm.cloudpaymentPay($jform, arArgs) }, 200); return !1 }
                initCP = { language: language };
                if (arArgs.applePaySupport && arArgs.applePaySupport == 'off') { initCP.applePaySupport = !1 }
            ...
            if (arArgs.cloudPayments && (arArgs.cloudPayments.recurrent || arArgs.cloudPayments.customerReceipt)) { arData.cloudPayments = arArgs.cloudPayments }
            var $jpopup = $jform.closest('.t-popup_show');
            if (!$jpopup || $jpopup.length == 0) { $jpopup = $jform.closest('.t706__cartwin_showed') }
            $jpopup.data('old-style', $jpopup.attr('style'));
            $jpopup.attr('style', 'z-index:100');
            window.tildaForm.orderIdForStat = arArgs.invoiceid;
            if (!arArgs.skin) { arArgs.skin = 'classic' }
            window.cloudpaymentshandler.charge({ publicId: arArgs.publicId, description: arArgs.description, amount: parseFloat(arArgs.amount), currency: currency, accountId: arArgs.accountId, invoiceId: arArgs.invoiceId, requireEmail: arArgs.requireEmail == !0 ? !0 : !1, email: arArgs.email, skin: arArgs.skin, data: arData }, function(options) {
                ...
                if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') { Tilda.sendEventToStatistics(virtPage, virtTitle, virtProduct, virtPrice) }
                if (arArgs.successurl > '') { window.setTimeout(function() { window.location.href = arArgs.successurl }, 300) }
                if ($jform.data('successmessage') > '') { $jform.find('.js-successbox').html($jform.data('successmessage')) } else { $jform.find('.js-successbox').html('') }
                $jform.data('successmessage', '');
                var successcallback = $jform.data('success-callback');
                ...
                $jform.data('successmessage', '');
                window.cloudpaymentshandler = !1;
                if (arArgs.failureurl > '') { window.location.href = arArgs.failureurl } else { $jpopup.find('.t706__cartwin-products').show();
                    $jpopup.find('.t706__cartwin-prodamount-wrap').show();
                    $jpopup.find('.t706__form-bottom-text').show();
                    $jform.find('.t-form__inputsbox').show(); try { tcart__lockScroll() } catch (e) {} }
            });
            return !1
        };
        window.tildaForm.sendStatAndShowMessage = function($jform, arArgs, sendStat) {
            ...
            if (arArgs.successurl > '') { window.setTimeout(function() { window.location.href = arArgs.successurl }, 300) }
            var successcallback = $jform.data('success-callback');
            if (successcallback && successcallback.length > 0) { eval(successcallback + '($jform)') }
        ...
        window.tildaForm.banktransferPay = function($jform, arArgs) {
            if (arArgs && arArgs.condition == 'fast') { window.tildaForm.sendStatAndShowMessage($jform, arArgs, !0) } else {
                if (arArgs && arArgs.html > '') {
                    ...
                    $('.t-banktransfer .t-popup__close').click(function() {
                        ...
                        try { if (typeof tcart__closeCart == 'function') { tcart__closeCart() } } catch (e) {}
                        return !1
                    });
                    ...
                    if ($jbankform.length > 0) {
                        ...
                        $jbankform.submit(function(e) {
                            ...
                            if (arErrors && arErrors.length > 0) { window.tildaForm.showErrors($jbankform, arErrors); return !1 }
                            $.ajax({
                                type: "POST",
                                ...
                                success: function(json) {
                                    ...
                                    if (!json) { json = { error: 'Unknown error. Please reload page and try again later.' } }
                                    if (json && json.error) { alert(json.error); return !1 }
                                    window.tildaForm.sendStatAndShowMessage($jform, arArgs, !0)
                                },
                                error: function(data) { $('body').removeClass('t-body_popupshowed');
                                    $jbankform.remove();
                                    alert(data) },
                                timeout: 15 * 1000
                            })
        ...
        window.tildaForm.closeSuccessPopup = function() {
            ...
            if ($popup.length > 0) {
                ...
                if (/iPhone|iPad|iPod/i.test(navigator.userAgent) && !window.MSStream) { window.tildaForm.unlockBodyScroll() }
                $popup.fadeOut('fast')
            }
        };
        window.tildaForm.lockBodyScroll = function() { var body = $("body"); if (!body.hasClass('t-body_scroll-locked')) { var bodyScrollTop = (typeof window.pageYOffset !== 'undefined') ? window.pageYOffset : (document.documentElement || document.body.parentNode || document.body).scrollTop;
        ...
        window.tildaForm.showSuccessPopup = function(message) {
            ...
            if ($popup.length == 0) { html = '<style media="screen"> .t-form-success-popup { display: none; position: fixed; background-color: rgba(0,0,0,.8); top: 0px; left: 0px; width: 100%; height: 100%; z-index: 10000; overflow-y: auto; cursor: pointer; } .t-body_success-popup-showed { height: 100vh; min-height: 100vh; overflow: hidden; } .t-form-success-popup__window { width: 100%; max-width: 400px; position: absolute; top: 50%; -webkit-transform: translateY(-50%); transform: translateY(-50%); left: 0px; right: 0px; margin: 0 auto; padding: 20px; box-sizing: border-box; } .t-form-success-popup__wrapper { background-color: #fff; padding: 40px 40px 50px; box-sizing: border-box; border-radius: 5px; text-align: center; position: relative; cursor: default; } .t-form-success-popup__text { padding-top: 20px; } .t-form-success-popup__close-icon { position: absolute; top: 14px; right: 14px; cursor: pointer; } @media screen and (max-width: 480px) { .t-form-success-popup__text { padding-top: 10px; } .t-form-success-popup__wrapper { padding-left: 20px; padding-right: 20px; } } </style>';
                ...
                $popup.click(function(e) { if (e.target == this) { window.tildaForm.closeSuccessPopup() } });
                $popup.find('.t-form-success-popup__close-icon').click(function(e) { window.tildaForm.closeSuccessPopup() });
                $(document).keydown(function(e) { if (e.keyCode == 27) { window.tildaForm.closeSuccessPopup() } }) }
            $('#tildaformsuccesspopuptext').html(message);
            $popup.fadeIn('fast');
            $('body').addClass('t-body_success-popup-showed');
            if (/iPhone|iPad|iPod/i.test(navigator.userAgent) && !window.MSStream) { setTimeout(function() { window.tildaForm.lockBodyScroll() }, 500) }
        };
        ...
        window.tildaForm.send = function($jform, btnformsubmit, formtype, formskey) {
            ...
            if ($jform.data('formcart') == 'y') { window.tildaForm.addPaymentInfoToForm($jform) }
            if (formtype == 2 || (!formtype && formskey > '')) {
                var $inputElem;
                $inputElem = $jform.find('input[name=tildaspec-cookie]');
                if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-cookie" value="">');
                    $inputElem = $jform.find('input[name=tildaspec-cookie]') }
                if ($inputElem.length > 0) { $inputElem.val(document.cookie) }
                $inputElem = $jform.find('input[name=tildaspec-referer]');
                if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-referer" value="">');
                    $inputElem = $jform.find('input[name=tildaspec-referer]') }
                if ($inputElem.length > 0) { $inputElem.val(window.location.href) }
                $inputElem = $jform.find('input[name=tildaspec-formid]');
                if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-formid" value="">');
                    $inputElem = $jform.find('input[name=tildaspec-formid]') }
                if ($inputElem.length > 0) { $inputElem.val($jform.attr('id')) }
                if (formskey > '') {
                    $inputElem = $jform.find('input[name=tildaspec-formskey]');
                    if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-formskey" value="">');
                        $inputElem = $jform.find('input[name=tildaspec-formskey]') }
                    if ($inputElem.length > 0) { $inputElem.val(formskey) }
                }
                $inputElem = $jform.find('input[name=tildaspec-version-lib]');
                if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-version-lib" value="">');
                    $inputElem = $jform.find('input[name=tildaspec-version-lib]') }
                if ($inputElem.length > 0) { $inputElem.val(window.tildaForm.versionLib) }
                $inputElem = $jform.find('input[name=tildaspec-pageid]');
                if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-pageid" value="">');
                    $inputElem = $jform.find('input[name=tildaspec-pageid]') }
                if ($inputElem.length > 0) { $inputElem.val($('#allrecords').data('tilda-page-id')) }
                $inputElem = $jform.find('input[name=tildaspec-projectid]');
                if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-projectid" value="">');
                    $inputElem = $jform.find('input[name=tildaspec-projectid]') }
                ...
                if (window.tildaForm.tildapayment && window.tildaForm.tildapayment.products) { d.push({ name: 'tildapayment', value: JSON.stringify(window.tildaForm.tildapayment) }) }
                $.ajax({
                    type: "POST",
                    url: $formurl,
                    data: d,
                    ...
                    success: function(json) {
                        ...
                        if (json && json.error) {
                            ...
                            if (!$errBox || $errBox.length == 0) { $jform.prepend('<div class="js-errorbox-all"></div>');
                                $errBox = $jform.find('.js-errorbox-all') }
                            var $allError = $errBox.find('.js-rule-error-all');
                            if (!$allError || $allError.length == 0) { $errBox.append('<p class="js-rule-error-all">' + json.error + '</p>');
                            ...
                            if (json && json.needcaptcha) { if (formskey) { tildaForm.addTildaCaptcha($jform, formskey); return } else { alert('Server busy. Please try again later.'); return } }
                            var formres = {};
                            if (json && json.results && json.results[0]) { var str = json.results[0];
                                str = str.split(':');
                                formres.tranid = '' + str[0] + ':' + str[1];
                                formres.orderid = (str[2] ? str[2] : '0'); if (formres.orderid > '' && formres.orderid != '0') { window.tildaForm.orderIdForStat = formres.orderid } } else { formres.tranid = '0';
                                formres.orderid = '0' }
                            $jform.data('tildaformresult', formres);
                            var virtPage = $jform.data('tilda-event-name') || '';
                            if (!virtPage || virtPage == '') { if ($jform.data('formcart') == 'y' && json && ((json.next && json.next.type && json.next.type != 'function') || !json.next)) { virtPage = '/tilda/' + $jform.attr('id') + '/payment/' } else { virtPage = '/tilda/' + $jform.attr('id') + '/submitted/' } }
                            var virtTitle = 'Send data from form ' + $jform.attr('id');
                            var virtPrice = 0;
                            var virtProduct = '';
                            if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') {
                                if (window.tildaForm.tildapayment && window.tildaForm.tildapayment.amount) { virtPrice = window.tildaForm.tildapayment.amount; if (parseFloat(window.tildaForm.tildapayment.amount) > 0) { virtTitle = 'Order ' + formres.orderid } } else { if ($jform.find('.js-tilda-price').length > 0) { virtPrice = $jform.find('.js-tilda-price').val(); if (parseFloat(virtPrice) > 0) { virtTitle = 'Order ' + formres.orderid } } }
                                Tilda.sendEventToStatistics(virtPage, virtTitle, virtProduct, virtPrice);
                                if (window.dataLayer) { window.dataLayer.push({ 'event': 'submit_' + $jform.attr('id') }) }
                            } else {
                                if (typeof ga != 'undefined' && ga) { if (window.mainTracker != 'tilda') { ga('send', { 'hitType': 'pageview', 'page': virtPage, 'title': virtTitle }) } }
                                if (window.mainMetrika > '' && window[window.mainMetrika]) { window[window.mainMetrika].hit(virtPage, { title: virtTitle, referer: window.location.href }) }
                                if (window.dataLayer) { window.dataLayer.push({ 'event': 'submit_' + $jform.attr('id') }) }
                            }
                            $jform.trigger('tildaform:aftersuccess');
                            if (formsendedcallback && formsendedcallback.length > 0) { eval(formsendedcallback + '($jform);') }
                            if (json && json.next && json.next.type > '') { var res = window.tildaForm.payment($jform, json.next);
                                successurl = ''; return !1 }
                    ...
                    error: function(error) {
                        ...
                        if (!$errBox || $errBox.length == 0) { $jform.prepend('<div class="js-errorbox-all"></div>');
                            $errBox = $jform.find('.js-errorbox-all') }
                        var $allError = $errBox.find('.js-rule-error-all');
                        if (!$allError || $allError.length == 0) { $errBox.append('<p class="js-rule-error-all"></p>');
                            $allError = $errBox.find('.js-rule-error-all') }
                ...
                if ($jform.data('is-formajax') == 'y') {
                    var d = {};
                    d = $jform.serializeArray();
                    if (window.tildaForm.tildapayment && window.tildaForm.tildapayment.amount) { d.push({ name: 'tildapayment', value: JSON.stringify(window.tildaForm.tildapayment) }) }
                    $.ajax({
                        type: "POST",
                        ...
                        success: function(html) {
                                    ...
                                    if (json && json.message) { if (json.message != 'OK') { $jform.find('.js-successbox').html(json.message) } } else {
                                        if (json && json.error) {
                                            ...
                                            if (!$errBox || $errBox.length == 0) { $jform.prepend('<div class="js-errorbox-all"></div>');
                                                $errBox = $jform.find('.js-errorbox-all') }
                                            var $allError = $errBox.find('.js-rule-error-all');
                                            if (!$allError || $allError.length == 0) { $errBox.append('<p class="js-rule-error-all">Unknown error. Please, try again later.</p>');
                                                $allError = $errBox.find('.js-rule-error-all') }
                            ...
                            var virtTitle = 'Send data from form ' + $jform.attr('id');
                            if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') { window.Tilda.sendEventToStatistics(virtPage, virtTitle, '', 0) } else {
                                if (typeof ga != 'undefined') { if (window.mainTracker != 'tilda') { ga('send', { 'hitType': 'pageview', 'page': virtPage, 'title': virtTitle }) } }
                                if (window.mainMetrika > '' && window[window.mainMetrika]) { window[window.mainMetrika].hit(virtPage, { title: virtTitle, referer: window.location.href }) }
                            }
                            $jform.trigger('tildaform:aftersuccess');
                            window.tildaForm.successEnd($jform, successurl, successcallback)
                    ...
                    return !1
                } else { var attraction = $jform.attr('action'); if (attraction.indexOf('forms.tildacdn.com') == -1) { btnformsubmit.data('form-sending-status', '3');
                        $jform.submit(); return !0 } else { return !1 } }
            }
        };
        $('.js-tilda-captcha').each(function() {
            if ($(this).attr('data-tilda-captchakey') > '') {
                if (window.tildaForm.isRecaptchaScriptInit === !1) { window.tildaForm.isRecaptchaScriptInit = !0;
                ...
                if ($('#' + idform + 'recaptcha').length == 0) { $(this).append('<div id="' + idform + 'recaptcha" class="g-recaptcha" data-sitekey="' + $(this).attr('data-tilda-captchakey') + '" data-callback="window.tildaForm.captchaCallback" data-size="invisible"></div>') }
        ...
        $('.r').on('focus', '.js-tilda-rule', function() { var str = $(this).attr('placeholder'); if (str && str.length > 0) { $(this).data('placeholder', str);
                $(this).attr('placeholder', '') } });
        $('.r').off('blur', '.js-tilda-rule');
        $('.r').on('blur', '.js-tilda-rule', function() { var str = $(this).data('placeholder'); if (str > '') { $(this).attr('placeholder', str);
        ...
        if ($jallforms.length > 0) { $jallforms.each(function() { if ($(this).data('formactiontype') != 1) { $(this).append('<div style="position: absolute; left: -5000px; bottom:0;"><input type="text" name="form-spec-comments" value="Its good" class="js-form-spec-comments"  tabindex="-1" /></div>') } }) }
        $('.r').find('.js-form-procces').each(function() { try { var formtype = $(this).data('formactiontype'); if (formtype == 2) { $(this).attr('action', '#') } } catch (e) { console.log(e) } });
        $('.r').off('submit', '.js-form-proccess');
        $('.r').on('submit', '.js-form-proccess', function(e) { var btnformsubmit = $(this).find('[type=submit]'); var btnstatus = btnformsubmit.data('form-sending-status'); if (btnstatus && btnstatus == 3) { btnformsubmit.data('form-sending-status', ''); return !0 } else { $(this).find('[type=submit]').trigger('click'); return !1 } });
        $('.r').on('dblclick', '.js-form-proccess [type=submit]', function(e) { e.preventDefault(); return !1 });
        ...
        $('.r').on('click', '.js-form-proccess [type=submit]', function(event) {
            ...
            if (btnstatus >= '1') { return !1 }
            var $activeForm = $(this).closest('form'),
                arErrors = !1;
            if ($activeForm.length == 0) { return !1 }
            ...
            if (window.tildaForm.showErrors($activeForm, arErrors)) { btnformsubmit.removeClass('t-btn_sending');
                btnformsubmit.data('form-sending-status', '0');
                btnformsubmit.data('submitform', ''); return !1 } else {
                ...
                if ($activeForm.find('.js-formaction-services').length == 0 && formtype != 1 && formskey == '') {
                    ...
                    if (!$errBox || $errBox.length == 0) { $activeForm.prepend('<div class="js-errorbox-all"></div>');
                ...
                if ($activeForm.find('.g-recaptcha').length > 0 && grecaptcha) {
                    ...
                    if (captchaid === undefined || captchaid === '') { var opts = { size: 'invisible', sitekey: $activeForm.data('tilda-captchakey'), callback: window.tildaForm.captchaCallback };
                        captchaid = grecaptcha.render($activeForm.attr('id') + 'recaptcha', opts);
                        $activeForm.data('tilda-captcha-clientid', captchaid) } else { grecaptcha.reset(captchaid) }
        ...
        try {
            ...
            if (TILDAPAGE_URL.toLowerCase().indexOf('utm_') !== -1) {
                ...
                if (typeof(TILDAPAGE_QUERY) == 'string') {
                    ...
                    for (i in arParams) { if (typeof(arParams[i]) != 'function') { arPair = arParams[i].split('='); if (arPair[0].substring(0, 4) == 'utm_') { TILDAPAGE_UTM = TILDAPAGE_UTM + arParams[i] + '|||' } } }
